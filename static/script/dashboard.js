...
function buildChart(pd) {
    ...
    setText('sum-profit',  fmt(sum(profits))  + ' so\'m');
...
function renderKPI(year) {
    ...
    // relative bars — max = max of all values
...
function renderToday(weeklyData) {
    // Today is the last point in weekly data
    const last = weeklyData.labels?.length - 1;
    if (last < 0) return;

    const todayLabel = document.getElementById('todayLabel');
    if (todayLabel && weeklyData.labels[last]) {
        todayLabel.textContent = weeklyData.labels[last];
    }

    const todayProfit  = weeklyData.profits?.[last]  || 0;
    const todaySale    = weeklyData.sales?.[last]    || 0;
    const todayPayment = weeklyData.payments?.[last] || 0;
    const todayExpense = weeklyData.expenses?.[last] || 0;

    setText('today-profit',  fmt(todayProfit)  + ' so\'m');
    setText('today-sale',    fmt(todaySale)    + ' so\'m');
    setText('today-payment', fmt(todayPayment) + ' so\'m');
    setText('today-expense', fmt(todayExpense) + ' so\'m');
}
...
async function loadDashboard() {
    ...
    try {
        ...
        buildChart(allData.chart[period]);
        renderToday(allData.chart.weekly);
        updateCacheInfo();
