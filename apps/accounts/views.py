...
from apps.accounts.forms import RegisterForm, UserEditForm, ProfileForm, PasswordChangeForm, LoginForm
...
class StaffListView(View):
    def get(self, request):
        ...
        fired  = users.filter(is_active=False)
...
class StaffFireView(View):
    def post(self, request, user_id):
        ...
        if user.is_superadmin:
            return JsonResponse({
                'status': 'error',
                'message': 'Super adminni ishdan bo\'yb bo\'lmaydi!'
            }, status=403)
...
from apps.products.models import Category, Region
